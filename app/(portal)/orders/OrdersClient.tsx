...
// Sentinel date used for "paused" recurring orders. Already excluded from
// admin + ShipStation by the scheduled_for date filter.
...
// Compute a display state that accounts for scheduled/paused overrides
  ...
  return (
    <div className="space-y-4">
      ...
      {orders.map((order) => {
        ...
        const isDateOpen = datePanelId === order.id
        ...
        return (
          <div key={order.id} className="bg-white rounded-xl border overflow-hidden"
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                {order.is_recurring && order.recurring_interval && (
                  <span className="text-xs px-2 py-0.5 rounded-full"
                    ...
                    🔄 {RECURRING_LABELS[order.recurring_interval] ?? order.recurring_interval}
                  </span>
                )}
              </div>
            ...
            {isEditing ? (
              ...
              /* ── Date picker panel (reschedule / resume) ── */
              <div className="px-5 py-4 space-y-3 bg-stone-50">
                ...
                <div className="flex gap-2">
                  <button
                    onClick={() => submitDateAction(order.id)}
                    disabled={busy}
                    className="text-sm font-semibold text-white px-4 py-2 rounded-lg disabled:opacity-50 transition-opacity hover:opacity-90"
                    style={{ backgroundColor: '#466c7e' }}
                  >
              ...
              <>
                ...
                {/* Action bar — only for pending orders */}
                {canManage && (
                  <div className="px-5 py-3 flex flex-wrap gap-2 border-t"
                    ...
                    <button
                      ...
                      disabled={busy}
                      className="text-xs font-medium px-3 py-1.5 rounded-lg border hover:bg-gray-50 disabled:opacity-50 transition-colors"
                      style={{ borderColor: '#d0d0d0', color: '#3b4858' }}
                    >
                      Edit
                    </button>

                    {/* Recurring-only actions */}
                    {order.is_recurring && (
                      paused ? (
                        /* Resume */
                        <button
                          onClick={() => openDatePanel(order, 'resume')}
                          disabled={busy}
                          className="text-xs font-medium px-3 py-1.5 rounded-lg border hover:bg-green-50 disabled:opacity-50 transition-colors"
                        ...
                        <>
                          {/* Reschedule */}
                          <button
                            onClick={() => openDatePanel(order, 'reschedule')}
                            disabled={busy}
                            className="text-xs font-medium px-3 py-1.5 rounded-lg border hover:bg-gray-50 disabled:opacity-50 transition-colors"
                            style={{ borderColor: '#d0d0d0', color: '#3b4858' }}
                          >
                          ...
                          <button
                            ...
                            disabled={busy}
                            className="text-xs font-medium px-3 py-1.5 rounded-lg border hover:bg-yellow-50 disabled:opacity-50 transition-colors"
                    ...
                    <button
                      ...
                      disabled={busy}
                      className="text-xs font-medium px-3 py-1.5 rounded-lg border hover:bg-red-50 disabled:opacity-50 transition-colors"
