...
function toWCOrder(order: DashboardOrder) {
  ...
  return {
    ...
    currency:         'USD',
    date_created:     wcDate(order.scheduled_for ? order.scheduled_for + 'T07:00:00Z' : order.created_at),
    date_modified:    wcDate(order.scheduled_for ? order.scheduled_for + 'T07:00:00Z' : order.created_at),
    date_created_gmt: wcDate(order.scheduled_for ? order.scheduled_for + 'T07:00:00Z' : order.created_at),
    date_modified_gmt:wcDate(order.scheduled_for ? order.scheduled_for + 'T07:00:00Z' : order.created_at),
